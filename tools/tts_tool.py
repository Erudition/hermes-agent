...
TTS_SCHEMA = {
    "name": "text_to_speech",
    "description": "Convert text to speech audio. Returns a MEDIA: path that the platform delivers as native audio. Compatible providers render as a voice bubble on Telegram; otherwise audio is sent as a regular attachment. In CLI mode, saves to ~/voice-memos/. Voice and provider are user-configured (built-in providers like edge/openai or custom command providers under tts.providers.<name>), not model-selected. Do NOT use this for normal conversation replies: on voice platforms (e.g. Discord voice channels) reply audio is already spoken automatically via low-latency streaming. Use only when the user explicitly asks to generate, save, or send an audio file or voice note.",
    "parameters": {
        "type": "object",
        "properties": {
            "text": {
